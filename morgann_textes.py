...
class Messages :    
    def divide_string(self, text: str) -> list[str]:                       # Fonction permettant de diviser une chaîne de caractères en plus petites chaînes 
        ...
        while True:
            if GameConfig.FONT_SIZE*len(text) > 20 :                                                  # Seuillage pour couper en lignes
    ...
    def display_message(self, text: str, position: v2, font: str, color: tuple[int, int, int], divide: bool) -> None:
        ...
        if divide == True:
            text_tab = Messages.divide_string(self, text)                    # Si la chaîne est trop longue on la découpe en plusieurs lignes
