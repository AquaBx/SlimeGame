...
class GameObject(IGameObject):
    ...
    def __init__(self, state: int, position: v2) -> None:
        ...
        self.taille: v2 = v2(GameConfig.BLOCK_SIZE, GameConfig.BLOCK_SIZE)
...
class Player(Dynamic):
    ...
    def __init__(self, state: int, position: v2, spritesheet: list[str]) -> None:
        animations = [ transform.scale(image.load(src), v2(GameConfig.BLOCK_SIZE, GameConfig.BLOCK_SIZE)) for src in spritesheet ]
        super().__init__(state, position, animations)
